...
namespace Canary.Core.Validators
{
    public static class DetailRecordValidator
    {
        ...
        public static List<ValidationMessage> Validate (DetailRecord rec)
        {
            ...
            try
            {
                ...
                {
                    errors.Add(new ValidationMessage() { LineNumber = parsedLineNumber, Message = $"'Account Number' field can only contain numeric, hyphens and space characters." });
                }
                ...
                if (!string.IsNullOrWhiteSpace(rec.WithholdingTaxIndicator) && !Regex.IsMatch(rec.WithholdingTaxIndicator, WITHHOLDING_TAX_INDICATOR_PATTERN))
                {
                    ...
                    errors.Add(new ValidationMessage() { LineNumber = parsedLineNumber, Message = $"'WithholdingTaxIndicator' contains illegal values. Allowed values are - N, W, X and Y." });
                }

                if (string.IsNullOrWhiteSpace(rec.TransactionCode) || !Regex.IsMatch(rec.TransactionCode, TRANSACTION_CODE_PATTERN))
                {
                    // TODO: Remove hardcoded values, should be stored in config
                    errors.Add(new ValidationMessage() { LineNumber = parsedLineNumber, Message = $"'Transaction Code' contains illegal values. Allowed values are - 13, 50, 51, 52, 53, 54, 55, 56 or 57." });
                }

                if (!CommonValidationRoutines.IsBsbNumber(rec.TraceBsb))
                {
                    errors.Add(new ValidationMessage() { LineNumber = parsedLineNumber, Message = $"'Trace BSB Number' field must be in format '999-999'." });
                }

                if (!CommonValidationRoutines.IsCentsString(rec.AmountInCents))
                {
                    errors.Add(new ValidationMessage() { LineNumber = parsedLineNumber, Message = $"'Amount' field must contain cents." });
                }

                if (!CommonValidationRoutines.IsAccountNumber(rec.TraceAccountNumber))
                {
                    errors.Add(new ValidationMessage() { LineNumber = parsedLineNumber, Message = $"'Trace Account Number' field can only contain numeric, hyphens and space characters." });
                }

                if (!CommonValidationRoutines.IsCentsString(rec.WithholdingTaxAmountInCents))
                {
                    errors.Add(new ValidationMessage() { LineNumber = parsedLineNumber, Message = $"'Amount of Withholding Tax' field must contain cents." });
                }
