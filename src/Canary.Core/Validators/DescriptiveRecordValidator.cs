...
{
    public class DescriptiveRecordValidator
    {
        ...
        public DescriptiveRecordValidator(ILogger logger)
        {
            _logger = logger;
        }

        public List<ValidationMessage> Validate(DescriptiveRecord rec)
        {
            ...
            try
            {
                ...
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Blank1' must be space filled." });
                }

                if (!CommonValidationRoutines.IsZeroFilledNumeric(rec.ReelSequenceNumber))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Reel Sequence Number' must be numeric zero filled e.g. '01'." });
                }

                if (!string.IsNullOrWhiteSpace(rec.Blank2))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Blank2' must be space filled." });
                }

                if (string.IsNullOrWhiteSpace(rec.UserPreferredSpecification))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Name of User Supplying File' field must be populated." });
                }

                if (!CommonValidationRoutines.IsLeftJustified(rec.UserPreferredSpecification, " "))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'User Preferred Specification' must be left justified and blank filled." });
                }

                if (!CommonValidationRoutines.IsZeroFilledNumeric(rec.UserIdentificationNumber))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'User Identification Number' must be numeric and zero filled." });
                }

                if (string.IsNullOrWhiteSpace(rec.Description))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Description' field must be populated." });
                }

                if (!CommonValidationRoutines.IsLeftJustified(rec.Description, " "))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Description' must be left justified, blank filled." });
                }

                if (!CommonValidationRoutines.IsValidDateString(rec.DateToBeProcessed))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Date to be processed' must be in DDMMYY format." });
                }

                if (!string.IsNullOrWhiteSpace(rec.Blank3))
                {
                    errors.Add(new ValidationMessage() { Type = ValidationMessage.MessageTypes.Error, LineNumber = parsedLineNumber, Message = $"'Blank3' must be space filled." });
                }
